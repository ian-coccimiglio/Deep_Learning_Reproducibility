...
def test_precision(size=1000, seed=42):
    ...
    return {"results": results, "hashes": hashes, "differences": differences}
...
if __name__ == "__main__":
    ...
    for size in [100, 1024, 10000]:
        results = test_precision(size=size)
        print(
            f"Testing with matrix size: {size}x{size}: {results['differences']['cpu_torch_vs_numpy']['mean_diff']}"
        )
